                        ...
                        <footer className='c-footer'>
                            <div className='o-layout'>
                                <div className='o-layout__item u-1/3@tablet'>
                                    ...
                                    <div>{__(LocalesNames.contact)} {config.email}</div>
                                    <div>Version: {version}</div>
                                    <div>{__(LocalesNames.weather_cright)}</div>
                                </div>
                                <div className='o-layout__item u-1/3@tablet'>
                                    <h4>{__(LocalesNames.international)}</h4>
                                    {config.internationalIds.map(code => <div key={code}><a href={getSchema('weahter', code) + '//' + getHost('weather', code) + links.weather.home({ ul: lang })}>{countryNames[code][lang]}</a></div>)}
                                </div>
                            </div>
                        </footer>
                    </div>
