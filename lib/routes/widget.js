...
route.get('/widget', function (req, res, next) {
	...
	res.locals.site.head.canonical = utils.canonical(currentCulture.country, links.weather.widget({
...
route.get('/widget/widget_html_script', function (req, res) {
	...
	if (scripttype === 'iframe') {
		data.push('<iframe src="' + utils.canonical(currentCulture.country, links.weather.widget.widgetFrame(req.query)) + '" scrolling="no" frameborder="0" style="border:none;overflow:hidden;height:' + height + 'px;width:' + width + 'px;" allowTransparency="true"></iframe>');
