...
internal.renderPlace = function(req, res) {
	...
	if (place.region) {
		res.locals.location.push({
			href: links.weather.places.byAdm1(place.region.admin1_code, {
				ul: lang
			}),
...
// place: /vremea/786654
route.get('/' + utils.route_prefix + '/:id(\\d+)', internal.route);
// place: /vremea/786654/10days
route.get('/' + utils.route_prefix + '/:id(\\d+)/10days', function(req, res, next) {
	req.params.days = 10;
