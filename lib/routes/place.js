...
internal.renderPlace = function(req, res, place) {
  ...
  res.locals.site.head.canonical = 'http://' + config.host + links.weather.place(place.id);
...
internal.getPlace = function(id) {
  var date = Date.now();
  return places.cacheAccess.getPlace(id).then(function(place) {
...
internal.getWeather = function(place) {
  return weather.access.getForecast(weather.forecast.formatSelector(place)).then(function(report) {
    if (report) {
