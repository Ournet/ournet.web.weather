...
internal.renderPlace = function(req, res, place, next) {
  ...
  var links = res.locals.links,
    ...
    country = res.locals.currentCulture.country,
  ...
  internal.getWeather(place).then(function(report) {
    ...
    if (!report) {
      utils.maxage(res, 0);
      core.logger.warn('No weather report', {
        place: {
          id: place.id,
          name: place.name,
    ...
    var entipicTimeImage;
    if (report) {
      entipicTimeImage = utils.entipicUrl(res.locals.util.weather.symbolName(report.days[0].times[0].symbol, 'en') + ' weather', 'b', 'en', 'us');
    }
    ...
    res.render('place', {
      ...
      entipicImage: internal.formatEntipicUrl(place, lang, country),
      entipicTimeImage: entipicTimeImage
    });
    //res.send(report);
  }).catch(next);
};
...
internal.formatPageInfo = function(place, res) {
  ...
  if (lang === 'sq') {
    info.pageTitle = core.util.albanianAtonic(info.pageTitle);
    info.description = core.util.albanianAtonic(info.description);
  }
...
internal.getWeather = function(place) {
  var selector = weather.forecast.formatSelector(place);
  //console.log(selector, place);
  return weather.CacheAccessService.instance.getForecast(selector);
};

internal.formatEntipicUrl = function(place, lang, country) {
  var name = place.getName(lang);
  if (place.region) {
    var shortadmname = places.util.shortAdm1Name(place.region, lang);

    if (!places.util.isTown(place) && !places.util.isCity(place)) {
      name = (place.name != place.region.name && !place.region.name.indexOf(place.name) > -1) ? util.format("%s, %s", name, shortadmname) : name;
    }
  }

  if (lang === 'ro') {
    name = core.util.romanianCorrect(name);
  }

  return utils.entipicUrl(name, 'b', lang, country);
};
