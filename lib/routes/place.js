...
internal.getPlace = function(id) {
  return places.CacheAccessService.instance.getPlace(id, true);
};
...
internal.formatPageInfo = function(place, res) {
  ...
  //if is adm1
  if (!adm1) {
    info.pageTitle = util.format(__('weather.weather_item_head_title_format'), inname, displayname);

    info.description =
      util.format(__('weather.weather_item_head_description_format'), inname + util.format(" (%s)", place.name), place.asciiname);

    info.title = util.format(__('weather.weather_title_format'), inname);
    ...
    if (!places.util.isTown(place) && !places.util.isCity(place)) {
      longname = (place.name != adm1.name && !adm1.name.indexOf(place.name) > -1) ? util.format("%s, %s", inname, shortadmname) : inname;
    }
    ...
    info.subTitle = util.format(__('weather.place_weather_details_info'), inname, util.format("%s, %s", place.asciiname, adm1.asciiname), displayname);
