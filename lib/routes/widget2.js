...
var widgetData = require('../data/widget2');
...
route.get('/widget2', function(req, res, next) {
	...
	res.locals.site.head.description = util.format(__('weather_on_your_site_info'), Data.places.util.inCountryName(currentCulture.countryName, currentCulture.language));

	res.locals.site.head.canonical = 'http://' + config.host + links.weather.widget2({
		ul: currentCulture.language
	});

	Data.places.cacheAccess.getPlace({
		id: config.capitalId,
		getRegion: true
	}).then(function(place) {
		res.render('widget2/' + utils.getRenderName(res, 'index'), {
			place: place
		});
	}).catch(next);
