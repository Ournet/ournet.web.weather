...
route.get('/widget2/widget_html_script', function (req, res) {
	...
	if (scripttype === 'iframe') {
		...
		data.push('<iframe src="' + res.locals.util.canonical(links.weather.widget2.widgetFrame(req.query)) + '" scrolling="no" frameborder="0" style="border:none;overflow:hidden;height:' + formatter.iframeHeight + 'px;width:' + req.query.w + 'px;" allowTransparency="true"></iframe>');
