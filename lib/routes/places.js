...
route.get('/' + config.prefix + '/places/:adm1', function(req, res, next) {
  ...
  function render(placeslist, region) {
    ...
    res.locals.title = util.format(__('weather.search_place_in_cn_format'), region.getName(lang), currentCulture.countryName);
...
internal.renderSearch = function(req, res, query) {
  ...
  client.search({
    ...
    if (resp && resp.hits.total == 1)
      return res.redirect(links.weather.place(resp.hits.hits[0]._id, {
