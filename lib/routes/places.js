...
route.get('/places', function (req, res, next) {
	...
	Data.get(viewdata)
		.then(function (result) {
			...
			res.locals.site.head.canonical = res.locals.util.canonical(links.weather.places({ ul: culture.language }));
...
internal.renderSearch = function (req, res, query, next) {
	...
	const viewdata = res.viewdata;
	viewdata.places = ['searchPlace', { country: currentCulture.country, query: query, limit: 9 }];
