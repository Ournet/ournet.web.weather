...
route.get('/places', function (req, res, next) {
	...
	viewdata.regions = ['admin1s', { limit: 93, country: culture.country }];
...
route.get('/places/:adm1', function (req, res, next) {
	...
	viewdata.places = ['admin1Places', { admin1Key: culture.country + '-' + adm1, limit: 90 }];
	viewdata.region = ['admin1', { country: culture.country, admin1: adm1 }];
...
internal.renderSearch = function (req, res, query, next) {
	...
	Data.get(viewdata)
		.then(result => {
			...
			if (places.length) {
				result.places = places = places.filter(item => item.featureClass !== 'A');
			}
			// console.log(places);
			diff = Date.now() - date;
			if (diff > 200) {
