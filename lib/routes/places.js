...
internal.renderSearch = function(req, res, query, next) {
  ...
  places.SearchService.instance.search({
    ...
    //console.log(places);
    if (places.length === 1 || (query.mode === 'one' && places.length > 0)) {
      // req.locals._events.push({
      //   category: 'redirect',
      //   action: 'found-place' + (query.mode && '-' + query.mode || ''),
      //   label: query.q.replace(/'/g, ''),
      //   value: 1
      // });
      core.logger.warn('Redirect: found place', {
        query: query,
        country: currentCulture.country
      });
    ...
    if (places.length === 0) {
      if (query.mode === 'one') {
        core.logger.warn('Redirect: NOT found place', {
          query: query,
          country: currentCulture.country
        });
        return res.redirect(links.weather.home({
        ...
        core.logger.warn('NOT found place', {
          query: query,
          country: currentCulture.country
        });
      }
