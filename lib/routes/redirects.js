...
route.get('/backup-ads/index.html', function(req, res) {
...
route.get('/:prefix/:id(\\d+)/10days', function(req, res) {
	const links = res.locals.links;
	const lang = res.locals.currentCulture.language;

	utils.maxageRedirect(res);

	res.redirect(301, links.weather.placeDays(req.params.id, 10, {
		ul: lang
	}));
});
...
route.get('/' + [utils.route_prefix, ':country', ':name', ':name2', ':id(\\d+)'].join('/'), function(req, res, next) {
