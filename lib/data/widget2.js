...
extern.format = function(config, formatter, query, links, __, lang) {
  formatter.format();
  ...
  return internal.getData(id).then(function(data) {
    ...
    for (var i = 0; i < query.days && i < data.weather.days.length; i++) {
      ...
      body.push('<li class="image"><div class="inner"><span title="' + symbolName + '" style="background-image: url(\'http://i.static.srsvc.com/root/img/icons/weather/48/' + time.symbol.number + '.png\')"></span></div></li>');
...
internal.getData = function(id) {
  return places.CacheAccessService.instance.getPlace({
    ...
    if (!place) return core.Promise.reject(new Error('Not found place: ' + id));
    return weather.CacheAccessService.instance.getForecast(weather.forecast.formatSelector(place)).then(function(report) {
      if (!report || !report.days) return core.Promise.reject(new Error('No weather report: ' + id));
      return {
        place: place,
        weather: report
      }
