...
exports.format = function(config, formatter, query, links, __, lang) {
	...
	return internal.getData(id)
		.then(function(data) {
			...
			var longtitle = util.format(__('weather_in_format'), Data.places.util.inPlaceName(data.place, lang));
			var title = longtitle;
			...
			for (var i = 0; i < query.days && i < data.weather.days.length; i++) {
				...
				if (day.times.length > 1) {
					maxtime = _.max(day.times, tValue);
					mintime = _.min(day.times, tValue);
				}
