	...
	return internal.getData(id)
		.then(function(data) {
			...
			for (var i = 0; i < query.days && i < data.weather.days.length; i++) {
				...
				var symbolName = Data.weather.util.symbolName(time.symbol, lang);
...
internal.getData = function(id) {
	return Data.places.access.place(id, { getRegion: true })
		.then(function(place) {
			if (!place) {
				return Promise.reject(new Error('Not found place: ' + id));
			}
			var selector = Data.weather.forecast.formatSelector(place);
			return Data.weather.access.getForecast(selector.key, selector)
